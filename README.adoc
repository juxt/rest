...
For example, here is a map that corresponds to a fictional *resource*. It
...
This makes sense for tranditional RPC-based protocols where each procedure is
...
While there is certainly some "implementation details behind the resource
interface (which are) hidden by the server", these implementation details are
not _only_ consideration when creating a REST service. The resource interface
itself needs to be implemented too!
...
The first step in processing a PUT request is to receive the "representation
enclosed in the request message payload" and check its validity.
